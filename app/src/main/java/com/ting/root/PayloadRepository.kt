...
data class VerifiedPayloads(
    ...
    val manager: File? = null,
    val useShellPreload: Boolean = false,
)
...
class PayloadRepository(private val context: Context) {
    ...
    fun s918u1RunnerTarget(snapshot: DeviceSnapshot): TargetProfile {
        require(snapshot.model == S918U1_MODEL && snapshot.buildId == S918U1_BUILD) {
            context.getString(R.string.s918u1_runner_mismatch)
        }
        return localTarget(
            snapshot = snapshot,
            profileId = S918U1_PROFILE_ID,
            displayName = context.getString(R.string.payload_source_s918u1_runner),
        )
    }

    fun s918u1RunnerPayloads(profile: TargetProfile): VerifiedPayloads {
        val directory = File(context.filesDir, "payloads/$S918U1_PROFILE_ID").apply { mkdirs() }
        val exploit = copyAsset(
            S918U1_ASSET_PREFIX + S918U1_PAYLOAD_NAME,
            File(directory, S918U1_PAYLOAD_NAME),
            0b100100100,
        )
        val helper = copyAsset(
            S918U1_ASSET_PREFIX + S918U1_HELPER_NAME,
            File(directory, S918U1_HELPER_NAME),
            0b111101101,
        )
        val kernelSu = copyAsset(
            S918U1_ASSET_PREFIX + S918U1_KSUD_NAME,
            File(directory, S918U1_KSUD_NAME),
            0b111101101,
        )
        val manager = copyAsset(
            S918U1_ASSET_PREFIX + S918U1_MANAGER_NAME,
            File(directory, S918U1_MANAGER_NAME),
            0b110100100,
        )
    ...
    private fun copyAsset(assetName: String, destination: File, mode: Int): File {
        if (!destination.exists()) {
            context.assets.open(assetName).use { input ->
                FileOutputStream(destination).use { output -> input.copyTo(output) }
            }
        }
        require(destination.length() > 0) { context.getString(R.string.s918u1_runner_assets_missing) }
    ...
    companion object {
        ...
        const val S918U1_PROFILE_ID = "dm3q-S918U1UES8FZG1-runner"
        private const val BUNDLED_LIBRARY = "libbs.so"
        private const val S918U1_MODEL = "SM-S918U1"
        private const val S918U1_BUILD = "BP4A.251205.006.S918U1UES8FZG1"
        private const val S918U1_ASSET_PREFIX = "s918u1/"
        private const val S918U1_PAYLOAD_NAME = "cve-2026-43499-app.so"
        private const val S918U1_HELPER_NAME = "cve-2026-43499-root"
        private const val S918U1_KSUD_NAME = "ksud-s25u-kdp"
        private const val S918U1_MANAGER_NAME = "KernelSU_Manager_v3.2.5_32525.apk"
